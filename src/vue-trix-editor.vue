<template>
  <div>
    <label
      v-if="label"
      :class="`trix-label ${focused ? 'trix-label--focused' : ''}`"
    >
      {{ label }}
    </label>
    <trix-editor
      ...
      @trix-file-accept="checkFile"
      @trix-attachment-add="acceptFile"
      @trix-attachment-remove="removeFile"
      @trix-initialize="init"
      @trix-focus="focused = true"
...
export default {
  ...
  methods: {
    hideToolbarAttachBtn() {
      this.$refs.trix.parentElement.querySelector(
...
trix-editor {
  ...
  [data-trix-mutable].attachment__caption-editor {
    &:focus {
      &::-moz-selection {
